...
def noRework(lib_df, wp_redo_df):
    ...
    # add empty columns that would have been generated by rework and are expected for later scripts
    lib_df[['Redo_whole_plate',	'Redo_Destination_Plate_Barcode','Redo_Destination_Well',	
    ...
    lib_df['Pool_source_plate'] = np.where(lib_df['Total_passed_attempts'] > 0, lib_df['Destination_Plate_Barcode'],"")
    ...
    # update project database with no rework info   
    updateProjectDatabase(lib_df, wp_redo_df)
    
    print("\nNo library rework is needeed.")
    print("\nRun SPS_conclude_FA_analysis_generate_ESP_smear_file.py to wrap up the FA analysis process\n")
