...
def getConstants():
    ...
    max_conc_vol = float(
        input("Enter the max CONCENTRATED volume used for individual libraries.  Should be <= half lib volume (default = 20uL): ") or 15)
...
def fillPoolingSheet(passed_df, target_pool_mass, max_conc_vol, max_dilut_vol, min_tran_vol):
...
passed_df = pd.read_csv(PROJECT_DIR / 'project_summary.csv', header=0,usecols=['Pool_source_plate', 'Pool_source_well',
