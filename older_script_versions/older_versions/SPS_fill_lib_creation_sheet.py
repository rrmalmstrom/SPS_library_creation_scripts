...
def mergeLibDataFiles(lib_df, clarity_df, clarity_lib_plate_id):
    ...
    # tmp_lib_df['Library Failure Mode'] = ""
    ...
    tmp_lib_df['Library Volume (ul)'] = np.where(tmp_lib_df['Total_passed_attempts'] > 0, 35,"")
    ...
    merged_df = merged_df.reindex(
     ...
     # merged_df['Aliquot Mass (ng)'] = np.where(
    #     merged_df['Library Name'].notnull(), 1.0, merged_df['Aliquot Mass (ng)'])

    merged_df.fillna('', inplace=True)
    
    
    # merged_df[['Pool_DNA_conc_ng/uL', 'Pool_nmole/L','Pool_Avg. Size']] = merged_df[['Pool_DNA_conc_ng/uL', 'Pool_nmole/L','Pool_Avg. Size']].astype(float)

   

    # clarity_info_df['Aliquot Mass (ng)'] = np.where(
    #     clarity_info_df['Library Name'].notnull(), 1.0, clarity_info_df['Aliquot Mass (ng)'])


    # ## probably need to add back to remove empty rows, but causes a probelm if clarity_index is empty
    # clarity_info_df.dropna(inplace=True)
