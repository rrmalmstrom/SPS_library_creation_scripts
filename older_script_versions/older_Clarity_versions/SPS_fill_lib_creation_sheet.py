...
def mergeLibDataFiles(lib_df, clarity_df, clarity_lib_plate_id):
    ...
    tmp_lib_df = tmp_lib_df.replace('',np.nan)
...
# read project_summary.db as a df
lib_df = readSQLdb()

# make a copy of project_summary df
summary_df = lib_df.copy()

# select a subset of columns
lib_df = lib_df[['sample_id','Destination Plate Barcode','Destination_Well','Total_passed_attempts','Pool_source_plate', 'Pool_source_well', 'Pool_Illumina_index_set', 'Pool_Illumina_index', 'Pool_DNA_conc_ng/uL', 'Pool_nmole/L', 'Pool_Avg. Size']]
...
# # make new df from project_summary.csv  so that clarity lib name, lims id, and lib plate id can be added to df
...
# updated the project_summary.csv file
updateProjectDatabase(updated_df)
