...
def addPoolClarityID(lib_df, pool_file):
    ...
    # merged_df = lib_df.merge(pool_df, how='outer', left_on=['Library Name', 'Destination Plate Barcode', 'Pool_source_well'], right_on=[
    #                          'clar_Lib_id', 'clar_Source Container Barcode', 'clar_Source Position'])
    
    # add clarity ids to lib_info
    merged_df = lib_df.merge(pool_df, how='outer', left_on=['Library Name', 'Destination Plate Barcode'], right_on=[
                             'clar_Lib_id', 'clar_Source Container Barcode'])
    ...
    merged_df['Pool_number'] = merged_df['Pool_number'].astype('Int64')
...
def addPippinInfo(my_lib_df):
    ...
    # for dealing with failed libs that will not be pooled but can't be removed from project summary
    if -99 in pool_list: 
        pool_list.remove(-99)
    ...
    my_lib_df['Pippin_Lane'] = my_lib_df['Pippin_Lane'].apply(lambda x: (-9,-9) if x is np.nan else x)
...
def makeTubeBarcodeFiles(my_lib_df):
    ...
    # get rid of any rows in df that have  -9 in first pippin lane. This is specific
    # work around for Hallma redo
    pippin_df = pippin_df[pippin_df['1st_Pippin_lane'] != -9]
    

    # remove duplicate rows, then sort by destination_tube_name, and reset the index
    pippin_df = pippin_df.drop_duplicates()
    
    pippin_df = pippin_df.sort_values(['Destination_Tube_Name'], ascending = True)
    ...
    my_dest_list = pippin_df['Destination_Tube_Name'].unique().tolist()
...
def updateProjectDatabase(final_df):
...
REWORK_DIR = POOLING_DIR / "D_pooling_and_rework"
...
# create df from project_summary.db
lib_df = readSQLdb()
...
# updated project_summary file
updateProjectDatabase(lib_df)
...
# Path(PROJECT_DIR /
#       "project_summary.csv").rename(ARCHIV_DIR / f"archive_project_summary{date}.csv")
# Path(ARCHIV_DIR / f"archive_project_summary_{date}.csv").touch()

# # create updated library info file
# lib_df.to_csv(PROJECT_DIR / 'project_summary.csv', index=False, header=True)
