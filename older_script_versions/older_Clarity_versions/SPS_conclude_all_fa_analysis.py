...
def updateLibInfo():
    ...
    lib_df['compare_dilution_factors'] = lib_df['Redo_dilution_factor'].equals(lib_df['Redo_dilution_factor_y'])
    
    # determine what to do if dilution factors differ
    if lib_df['compare_dilution_factors'].any() == False:
        ...
        if (val == 'Y' or val == 'y'):
            ...
            print('\n\n Ok, aborting script. Please resolve issue with dilution factor in thresholds file \n\n')
            sys.exit()
        else:
            print("\n\n Sorry, you must choose 'Y' or 'N' next time. \n\nAborting\n\n")
            sys.exit()
    
    
    # remove column with dilution_factor read from thresholds file and column comparing dilution factors
    lib_df.drop(columns=['Redo_dilution_factor_y','compare_dilution_factors'], inplace=True)
...
updateProjectDatabase(final_df)
